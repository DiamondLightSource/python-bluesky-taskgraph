...
def _format_task(task, dependencies, inputs, outputs):
    return f"{task.name}: depends on: {dependencies}, has inputs: {inputs}, has outputs: {outputs}"


# TODO: Likely other useful inbuilt methods to override
class TaskGraph:
    """
    A TaskGraph contains a mapping of task to all of the tasks that must be complete before it starts (the graph), as
    well as a mapping of task to the *names* of inputs that it will receive and *names* of outputs that it will provide
    This mapping is done by the TaskGraph to prevent accidental shadowing by outputs: e.g. a device named "wavelength"
    and a value named the same provided by another task.
    """
    ...
    def __str__(self) -> str:
        ...
        dependencies = ([dependency.name for dependency in self.graph.get(key, [])] for key in tasks)
        inputs = (self.inputs.get(key, []) for key in tasks)
        outputs = (self.outputs.get(key, []) for key in tasks)
        return str([_format_task(*task) for task in zip(tasks, dependencies, inputs, outputs)])

    def __len__(self):
        return len(self.graph)

    # Makes all tasks in this graph depend on the completion of all tasks within another graph
    # And adds the other graph to this graph.
    # Returns the newly constructed graph to allow chaining of this method
